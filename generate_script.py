...
""",
    ...
    "映画紹介": {
        "system": "あなたは映画紹介の台本作成の専門家です。絵文字、記号、装飾は一切禁止です。日本語のテキストと句読点のみで台本を書いてください。",
        "prompt": """
【絶対厳守ルール】
1. 絵文字は絶対に使用しない（😀🎬🔥などNG）
2. 記号は使用しない（！？以外）
3. 1行は必ず1文のみ（複数の文を1行にまとめない）
4. 1文は15-25文字程度に抑える
5. 必ず7-10行で構成する
6. フォロー促進、コメント促進の文言は絶対に含めない
...
Netflix、Prime Video、Disney+などで観られる映画を1つ紹介する台本を作成してください。
...
【悪い例（これを避ける）】
この映画マジでヤバい🔥
超おすすめの作品なんだけど、ストーリーが複雑で何度見ても楽しめるし、俳優の演技も最高なんだよね！

上記のルールを厳密に守り、良い例のように1行1文で改行し、絵文字を使わず日本語のみで台本を作成してください。
台本の内容だけを出力してください。説明や注釈は不要です。
""",
...
それでは、{account_name}向けの台本を書いてください:"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Unicode絵文字の範囲をOR結合で指定（日本語文字の誤削除を防ぐ）
            emoji_pattern = re.compile(
                r"[\U0001F600-\U0001F64F]|"  # 顔の絵文字
                r"[\U0001F300-\U0001F5FF]|"  # 記号とピクトグラム
                r"[\U0001F680-\U0001F6FF]|"  # 交通と地図記号
                r"[\U0001F1E0-\U0001F1FF]|"  # 旗
                r"[\U0001F900-\U0001F9FF]|"  # 補助記号
                r"[\U0001FA00-\U0001FA6F]|"  # 拡張A
                r"[\U0001FA70-\U0001FAFF]|"  # 拡張B
                r"[\U00002702-\U000027B0]",  # その他の記号
                flags=re.UNICODE
            )
