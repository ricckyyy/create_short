...
このガイドでは、GitHub Actionsを使って動画を自動生成する方法を**最短で**説明します。
...
### セットアップ
1. セルフホストランナーをセットアップ（上記参照）
2. ワークフローは自動で有効化されます
3. PRを作成すると自動実行されます

### 無効化する場合
`.github/workflows/pr-video-generation.yml` を削除または無効化してください。
